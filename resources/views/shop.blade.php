  ...
  <section class="shop-main container d-flex pt-4 pt-xl-5">
    <div class="shop-sidebar side-sticky bg-body" id="shopFilter">
      <div class="aside-header d-flex d-lg-none align-items-center">
        <h3 class="text-uppercase fs-6 mb-0">Filtern nach</h3>
        ...
        <div class="accordion-item mb-4">
          ...
          <div
            ...
            <div class="price-range__info d-flex align-items-center mt-2">
              <div class="me-auto">
                <span class="text-secondary">Mindestens:&nbsp;</span>
                <span class="price-range__min">${{ $min_price }}</span>
              </div>
              <div>
                <span class="text-secondary">Höchstens:&nbsp;</span>
...
<div class="shop-list flex-grow-1">
          ...
          <div class="slide-split_text position-relative d-flex align-items-center" style="background-color: #f5e6e0;">
            <div class="slideshow-text container p-3 p-xl-5">
              <h2 class="text-uppercase section-title fw-normal mb-3 animate animate_fade animate_btt animate_delay-2">
                Damen-<br /><strong>ACCESSOIRES</strong>
              </h2>
              <p class="mb-0 animate animate_fade animate_btt animate_delay-5">
                Accessoires sind der beste Weg, um Ihren Look zu aktualisieren. Setzen Sie neue Akzente mit frischen Stilen und Farben oder entscheiden Sie sich für zeitlose Stücke.
              </p>
            </div>
          </div>
          <div class="slide-split_media position-relative">
            <div class="slideshow-bg" style="background-color: #f5e6e0;">
              <img loading="lazy" src="assets/images/shop/shop_banner3.jpg" width="630" height="450"
                alt="Damen-Accessoires" class="slideshow-bg__img object-fit-cover" />
            </div>
          </div>
        </div>
      </div>
  ...
  <div class="d-flex justify-content-between mb-4 pb-md-2">
    <div class="breadcrumb mb-0 d-none d-md-block flex-grow-1">
      <a href="{{route('home.index')}}" class="menu-link menu-link_us-s text-uppercase fw-medium">Startseite</a>
    ...
    <div class="shop-acs d-flex align-items-center justify-content-between justify-content-md-end flex-grow-1">
      ...
      <select class="shop-acs__select form-select w-auto border-0 py-0 order-1 order-md-0" style="margin-right:20px;" aria-label="Seitengröße" id="pagesize" name="pagesize">
        <option value="12" {{$size=='12'? 'selected':''}}>Anzeigen</option>
      ...
      <form id="frmFilter" method="GET" action="{{ route('shop.index') }}">
        ...
        {{-- Sortierung --}}
        <select id="sorting" name="sorting" class="...">
          <option value="default"   {{ $sorting=='default'?'selected':'' }}>Standard</option>
          <option value="date"      {{ $sorting=='date'?'selected':'' }}>Neueste</option>
          <option value="price"     {{ $sorting=='price'?'selected':'' }}>Preis ↑</option>
          <option value="price-desc"{{ $sorting=='price-desc'?'selected':'' }}>Preis ↓</option>
        </select>
      </form>
      ...
      <div class="col-size align-items-center order-1 d-none d-lg-flex">
        <span class="text-uppercase fw-medium me-2">Ansicht</span>
      ...
      <div class="shop-filter d-flex align-items-center order-0 order-md-3 d-lg-none">
        <button class="btn-link btn-link_f d-flex align-items-center ps-0 js-open-aside" data-aside="shopFilter">
          ...
          <span class="text-uppercase fw-medium d-inline-block align-middle">Filtern</span>
        </button>
      </div>
    </div>
  </div>
        ...
        <div class="product-card mb-3 mb-md-4 mb-xxl-5">
          <div class="pc__img-wrapper">
            ...
            @if(Cart::instance('cart')->content()->where('id', $product->id)->count() > 0)
              <a href="{{ route('cart.index') }}"
                 class="pc__atc btn anim_appear-bottom position-absolute border-0 text-uppercase fw-medium btn-warning mb-3">
                Zum Warenkorb
              </a>
            @else
          ...
          <div class="pc__info position-relative">
            ...
            <div class="product-card__review d-flex align-items-center">
              ...
              <span class="reviews-note text-lowercase text-secondary ms-1">8k+ Bewertungen</span>
            </div>
            ...
            @if(Cart::instance("wishlist")->content()->Where('id',$product->id)->count()>0)
              <form method="POST" action="{{route('wishlist.remove',['rowId'=>Cart::instance("wishlist")->content()->Where('id',$product->id)->first()->rowId])}}">
                @csrf
                @method("DELETE")
                <button type="submit" class="pc__btn-wl position-absolute top-0 end-0 bg-transparent border-0 filled-heart" title="Von der Wunschliste entfernen">
            ...
            @else
              <form method="POST" action="{{route('wishlist.add')}}">
                ...
                <button type="submit" class="pc__btn-wl position-absolute top-0 end-0 bg-transparent border-0" title="Zur Wunschliste hinzufügen">
                  <svg width="16" height="16" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <use href="#icon_heart" />
                  </svg>
                </button>
            ...
            <button class="pc__btn-wl position-absolute top-0 end-0 bg-transparent border-0 js-add-wishlist" title="Zur Wunschliste hinzufügen">
              <svg width="16" height="16" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                <use href="#icon_heart" />
              </svg>
            </button>
...
<script>
  $(function () {
    ...
    $('input[name="brands"]').on('change', function () {
      const brands = $('input[name="brands"]:checked').map((_, el) => el.value).get().join(',');
    ...
    $('input[name="categories"]').on('change', function () {
      const categories = $('input[name="categories"]:checked').map((_, el) => el.value).get().join(',');
    ...
    $("[name='price_range']").on("change", function () {
      ...
      console.log('Slider geändert:', values); // ← Testausgabe
      $("#frmfilter").submit();
    });
  });
</script>
